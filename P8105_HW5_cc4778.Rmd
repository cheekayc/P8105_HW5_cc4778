...
sim_try = 
  expand_grid(   
    ...
    iteration = 1:5) %>%   
  mutate(
    estimate_measures = map(true_mean, try_try)) %>% 
...
ggplot(sim_one, aes(x = true_mean, y = average_mean_hat)) +
  geom_point() +
  geom_line() + 
  geom_line(data = sim_two, color = "red", alpha = 0.4) +
  geom_point(data = sim_two, color = "red", alpha = 0.4)
