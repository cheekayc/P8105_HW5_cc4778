...
*Oklahoma*. Therefore, I am pretty sure that there is an error in the
data and decided to fix that error by changing `Tulsa, AL` to
`Tulsa, OK`.
...
are unsolved; save the output as an R object, apply the broom::tidy to
this object and pull the estimated proportion and confidence intervals
from the resulting tidy dataframe.

From the table above, we know that there are a total of **2827**
homicide cases and **1825** unsolved homicide cases in **Baltimore, MD**
